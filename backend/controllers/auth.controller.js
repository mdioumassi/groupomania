...
module.exports.signUp = async (req, res, next) => {
    ...
    db.query(sql, [pseudo, email, cryptedPassword], (err, result) => {
        if (err) {
            ...
            res.status(201).json({'email': email, 'pseudo': pseudo});
        }
    });
};
...
module.exports.signIn = (req, res, next) => {
    ...
    db.query(sql, [email], (err, result) => {
        if (err) {
            ...
            if (result.length == 0) {
                res.status(400).json({message: 'EMAIL_NOT_FOUND', errorPassword: ''});
            } else {
                bcrypt.compare(password, result[0].password)
                    .then(valid => {
                        if (!valid) {
                            res.status(400).json({message: 'INVALID_PASSWORD', errorEmail: ''});
                        } else {
